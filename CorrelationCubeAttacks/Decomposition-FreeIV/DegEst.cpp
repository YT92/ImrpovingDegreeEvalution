...
int DegEstRec(int cube[], int dim, char ***upA, char ***upB, char ***upC, int sizeA[], int sizeB[], int sizeC[], int vfA[], int vfB[], int vfC[], int round, int N0)
{
	...
	while(recurflag==1)
	{
		memset(findeglist,0,sizeof(int)*10000);
		for (i = 0; i < dim; i++)
		{
			if (testflag[i] == 0)
			{
...
int DegEstFreeNC(int cube[], int dim, int round, int N0, int pos, int posr,int freenc)
{
	int Da[10000], Db[10000], Dc[10000], Dav[10000], Dbv[10000], Dcv[10000];
	int Da_t[10000], Db_t[10000], Dc_t[10000];
	int baisnum[1000]={0};
	...
	memset(baisnum,0,sizeof(int)*1000);
	
	decompositionFreeNC(cube,dim,freenc,upA,upB,upC,sizeA,sizeB,sizeC,vfA,vfB,vfC,pos,posr,baisnum,N0);
...
int DegEstFreeNCV2(int cube[], int dim, int round, int N0, int pos[], int posr[],int posnum,int freenc)
{
	...
	decompositionFreeNCV2(cube,dim,freenc,upA,upB,upC,sizeA,sizeB,sizeC,vfA,vfB,vfC,pos,posr,posnum,baisnum,N0);
...
void GetandTestXFreeIV()
{
	...
	while (c != EOF)
	{
		...
		fprintf(fpout,"\nFree Non Cube Variable: \n");
		flag=0;
		//fclose(fpout);
		//get 
		//
		memset(Cubeflag,0,sizeof(int)*1000);
		for(i=0;i<dim;i++)
		{
			Cubeflag[X[i]]=1;
		}
		memset(NonCube,0,sizeof(int)*1000);
		ncnum=0;
		for(i=0;i<80;i++)
		{
			if(Cubeflag[i]==0)
		...
		for(i=0;i<ncnum;i++)
		{
			//printf("%d %d:",i,NonCube[i]);
			flag=DegEstFreeNC(X, dim, roundnum,N0,pos[l],posr[l],NonCube[i]);
			//fopen_s(&fpout, "Res.txt", "a+");
			if (flag == 0)
			{
				//printf("Usatisified!!! Plaese try another cube\n\n");
			...
			{
				//输出它的基
				printf( "Free IV found!!! %d\n\n", NonCube[i]);
				fprintf(fpout, "%d ", NonCube[i]);
			}
			//printf("\n");
...
void GetandTestXFreeIVV2()
{
	...
	c = fgetc(fp);
	int X[80] = { 0 };
	/*int pos[1000][10]={{2,2,2},{2,2,2},{2,2,2},{2,2,2}};
	int posr[1000][10]={{90,92,94},{90,92,94},{90,92,94},{121,123,125}};
	int posnum[1000]={3,3,3,3};*/
	...
	while (c != EOF)
	{
		...
		{
			//printf("%d %d:",i,NonCube[i]);
			flag=DegEstFreeNCV2(X, dim, roundnum,N0,pos[l],posr[l],posnum[l],NonCube[i]);
			//fopen_s(&fpout, "Res.txt", "a+");
			if (flag == 0)
			{
				//printf("Usatisified!!! Plaese try another cube\n\n");
